## Perturbformer-V2: Drug_SMILES_Embedding Model
import os
import re
import random
import numpy as np
...
MODEL_DIR = os.path.join(BASE_DIR, "morgan_embedding")
...
## 8. Model
class MLPFingerprintModel(nn.Module):
    """
    input:
    drug_fp: [batch, 2048]
    cell_idx: [batch]
    ...
    pred: [batch, 978]
    """
    def __init__(self, fp_dim, num_cells, out_dim, drug_emb_dim=128, cell_emb_dim=32, hidden_dim=512,):
        super().__init__()
...
best_model_path = os.path.join(MODEL_DIR, "mlp_fingerprint_best.pt")
