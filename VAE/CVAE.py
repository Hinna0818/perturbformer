## Perturbformer-V5: Drug_SMILES_Embedding + conditional VAE model
import os
import re
import random
import numpy as np
...
## 8. Model
class StandardCVAE(nn.Module):
    """
    Standard Conditional VAE for L1000 perturbation signature generation.

    Training:
        encoder input: y + condition
        decoder input: z + condition
    """
    ...
    def __init__(
        ...
        self.condition_dim = drug_emb_dim + cell_emb_dim + 2

        encoder_input_dim = out_dim + self.condition_dim
        ...
        decoder_input_dim = z_dim + self.condition_dim
    ...
    def build_condition(self, drug_fp, cell_idx, numeric):
        drug_vec = self.drug_encoder(drug_fp)
        cell_vec = self.cell_emb(cell_idx)
        condition = torch.cat([drug_vec, cell_vec, numeric], dim=1)
        return condition

    def encode(self, y, condition):
    ...
    def decode(self, z, condition):
        ...
        pred = self.decoder_net(decoder_input)
        return pred

    def forward(self, drug_fp, cell_idx, numeric, y):
        condition = self.build_condition(drug_fp, cell_idx, numeric)
        mu, logvar = self.encode(y, condition)
        z = self.reparameterize(mu, logvar)
        pred = self.decode(z, condition)

        return pred, mu, logvar, z

    def generate(self, drug_fp, cell_idx, numeric, n_samples=1):
        """
        Generate expression signatures from condition by sampling z ~ N(0, 1).

        Output:
            preds: [n_samples, batch_size, out_dim]
        """
        condition = self.build_condition(drug_fp, cell_idx, numeric)
        ...
        for _ in range(n_samples):
            z = torch.randn(batch_size, self.z_dim, device=drug_fp.device)
            pred = self.decode(z, condition)
            preds.append(pred)
...
def vae_loss(pred, target, mu, logvar):
    recon = mse_loss(pred, target) + 0.2 * pearson_loss(pred, target)
...
def evaluate_model(model, loader):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred, mu, logvar, z = model(drug, cell, num, y)
            loss, recon, kl = vae_loss(pred, y, mu, logvar)
...
def evaluate_generation(model, loader, n_samples=20):
    """
    Evaluate virtual-cell generation ability.

    This function does NOT use true y as encoder input.
    It only uses condition: drug_fp + cell_idx + numeric.
    
    For each condition, sample z ~ N(0, 1) multiple times,
    decode generated signatures, average them, and compare with true y.
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Generate signatures from condition only
            gen_preds = model.generate(
                drug,
                cell,
                num,
                n_samples=n_samples
            ...
            # gen_preds: [n_samples, batch_size, 978]
            pred_mean = gen_preds.mean(dim=0)
...
best_model_path = os.path.join(MODEL_DIR, "standard_cvae_best.pt")
...
for epoch in range(1, EPOCHS + 1):
    ...
    train_recon = train_recon / len(train_loader)
...
# 13.1 Reconstruction evaluation
# This uses true y as encoder input.
# It evaluates whether the CVAE can reconstruct known signatures.
recon_metrics = evaluate_model(model, test_loader)

print(
    f"[Reconstruction] "
    f"Test Loss: {recon_metrics['loss']:.4f} | "
    f"Test Recon: {recon_metrics['recon']:.4f} | "
    f"Test KL: {recon_metrics['kl']:.4f} | "
    f"Test RMSE: {recon_metrics['rmse']:.4f} | "
    f"Test Pearson: {recon_metrics['pearson']:.4f} | "
    f"Test Spearman: {recon_metrics['spearman']:.4f}"
)

# 13.2 Generation evaluation
# This does NOT use true y as encoder input.
# This is the virtual-cell prediction/generation ability.
...
print(
    f"[Generation / Virtual Cell] "
    f"Test RMSE: {gen_metrics['rmse']:.4f} | "
    f"Test Pearson: {gen_metrics['pearson']:.4f} | "
    f"Test Spearman: {gen_metrics['spearman']:.4f}"
)
